...
@section('content')
                    ...
                    @foreach($event->media as $file)
                        <tr id="file{{$file->id}}">
                            ...
                            <td style="text-align: center;">
                                @if($file->mime == 'image')
                                    <a class="image-popup" href="{{ asset('storage/events/full/'.$file->filename)}}" >
                                        <img class="max100" src="{{ asset('storage/events/display/'.$file->filename)}}">
                                    </a>
                                @endif
                            </td>
                            <td>
    ...
    <div class="text-center mt40">
        <a class="btn btn-primary" href="{{url('admin/eventi')}}">Torna Agli Eventi</a><a class="btn btn-primary" href="{{$event->url}}">Vedi Evento</a>
    </div>
...
<script>
        ...
        Dropzone.options.dropzoneForm = {
            ...
            sending: function(file, xhr, formData) {
                formData.append("mediable_id", "{{$event->id}}");
