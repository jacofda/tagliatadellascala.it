...
@section('content')
                    ...
                    @foreach($article->media as $file)
                        <tr id="file{{$file->id}}">
                            ...
                            <td style="text-align: center;">
                                @if($file->mime == 'image')
                                    <a class="image-popup" href="{{ asset('storage/articles/full/'.$file->filename)}}" >
                                        <img class="max100" src="{{ asset('storage/articles/display/'.$file->filename)}}">
                                    </a>
                                @endif
                            </td>
                            <td>
    ...
    <div class="text-center mt40">
        <a class="btn btn-primary" href="{{url('admin/articoli')}}">Torna Agli Articoli</a><a class="btn btn-primary" href="{{$article->url}}">Vedi Articolo</a>
    </div>
...
<script>
        ...
        Dropzone.options.dropzoneForm = {
            ...
            sending: function(file, xhr, formData) {
                formData.append("mediable_id", "{{$article->id}}");
